...
def spectral_loss(expected, actual, mag_weight=1.0, phase_weight=1.0):
    ...
    en = tf.random_normal(shape=tf.shape(exp), mean=0.0, stddev=0.00001, dtype=tf.float32);
    an = tf.random_normal(shape=tf.shape(act), mean=0.0, stddev=0.00001, dtype=tf.float32);
...
loss = tf.losses.absolute_difference(labels, labels_predicted)
#[loss, es, as1] = spectral_loss(labels, labels_predicted, 1.0, 3.0)
lr = .1
...
sess.run(init)
writer = tf.summary.FileWriter("trainlog_22k_MAE", sess.graph);
...
for epo in range(EPOCHS):
    ...
    while True:
        ...
        try:
            ...
            break
    modelName = os.getcwd() + '/models/unet22khzMAE/' + str(epo) + 'loss:' + str(loss_np) + '.ckpt'
