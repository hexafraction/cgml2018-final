...
def WaveUNet(features):
    ...
    convFilters = 5 # num extra filters per layer
    convStride = 1   
    convPadding = 'valid'   # 'valid' means none (switch to valid at some point
    LAYERS = 3 	    #how deep is  your love (for source separation using U nets)
    ...
    for i in range(LAYERS):
        ...
        l1 = tf.layers.conv1d(l1,convFilters*(i+1),down_kernel_size,padding = convPadding,activation = tf.nn.leaky_relu )
    ...
    for i in reversed( range(LAYERS)):
        ...
        l1 = tf.layers.conv1d(l1,convFilters*(i+1),up_kernel_size,padding = convPadding,activation= tf.nn.leaky_relu)
            #print('l1\t\t',l1.shape)
...
features  = tf.concat([mix, drums, bass, vocals], 2)  #tf.placeholder(tf.float32, [None,16384,2])  # Should get batch size by 2 array of labels
labels = accomp #tf.placeholder(tf.float32, [None,16384,2])     # Revisit this idk if it's right
...
sess.run(init)
writer = tf.summary.FileWriter("trainlogMicro", sess.graph);
...
for epo in range(EPOCHS):
    ...
    modelName = os.getcwd()+'/models/microModels/'+str(epo)+'loss:'+str(loss_np)+'.ckpt'
