...
sess.run(init)
writer = tf.summary.FileWriter("trainlog_22k_PhaseUNNORM", sess.graph);
...
for epo in range(EPOCHS):
    ...
    while True:
        ...
        try:
            ...
            #rint(as_);
            if k % 5 == 0:
                sys.stderr.write("Loss: " + str(loss_np)+'\n')
            writer.add_summary(summ, k + epo * NUM_ITER);
        except tf.errors.OutOfRangeError:
            break
    modelName = os.getcwd() + '/models/unet22khzPhaseUNNORM/' + str(epo) + 'loss:' + str(loss_np) + '.ckpt'
